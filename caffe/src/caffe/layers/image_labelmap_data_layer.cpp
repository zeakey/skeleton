...
void ImageLabelmapDataLayer<Dtype>::load_batch(LabelmapBatch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for(int k = 0;k < this->transformed_labelmap_.count(); k++){
      if(transformat_data[k] < 1 || transformat_data[k] >= 180)
        transformat_data[k] = Dtype(0);
      
      else if(transformat_data[k] >= 1 && transformat_data[k] < 10)
        transformat_data[k] = Dtype(1);
      
      else if(transformat_data[k] >= 10 && transformat_data[k] < 32)
        transformat_data[k] = Dtype(2);
      
      else if(transformat_data[k] >= 32 && transformat_data[k] < 80)
        transformat_data[k] = Dtype(3);
      
      else if(transformat_data[k] >= 80 && transformat_data[k] < 180)
        transformat_data[k] = Dtype(4);
